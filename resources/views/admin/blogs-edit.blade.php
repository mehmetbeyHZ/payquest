...
@section('content')
    <div class="container">
        ...
        <div class="mt-3">
            ...
            <form action="#" id="imageUploadForm">
                <div class="file-field input-field d-flex">
                    ...
                    <div class="ml-1" style="margin-top: auto;margin-bottom: auto">
                        <label class="file-path validate" id="form_image_uri">{{ $blog->image ?? '...' }}</label>
            ...
            <form action="{{ route('admin.blogs.edit.post',['id' => $blog->id]) }}" method="post" class="submit">
                <input type="hidden" name="image" id="blog_image" value="{{ $blog->image }}">
                <input type="text" name="title" placeholder="Başlık" value="{{ $blog->title }}">
                <textarea id="summernote" name="text">{!! $blog->text !!}</textarea>
                <select name="is_published" id="" class="browser-default mt-1">
                    <option value="1" @if($blog->is_published === 1) selected @endif>Yayınla</option>
                    <option value="2" @if($blog->is_published === 2) selected @endif>Taslak Olarak Kaydet</option>
                </select>
