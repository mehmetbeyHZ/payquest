    ...
    <div class="container">
                ...
                <div class="d-flex">
                    <div style="width: 100px; height: 100px; border: 2px solid#dbdbdb;border-radius: 4px;" >
                        <div style="width: 100px; height: 100px;max-height: 100%;max-width: 100%;">
                            <img src="{{ $competition->competition_image }}" alt="" id="preview_image"
                        ...
                        <form action="#" id="imageUploadForm">
                            <div class="file-field input-field d-flex">
                                ...
                                <div class="ml-1" style="margin-top: auto;margin-bottom: auto; max-width: 40%;word-wrap: break-word;overflow:hidden; text-overflow:  ellipsis;white-space: nowrap">
                                    <label class="file-path validate" id="form_image_uri">@if($competition->competition_image) {{ $competition->competition_image }} @else ... @endif</label>
        ...
        <div class="card wg_shadow">
            <div class="card-content">
                ...
                <form action="{{ route('admin.competitions.update.post',['id' => $competition->competition]) }}" method="post" class="submit">
                    @csrf
                    <input type="hidden" id="competition_image" value="{{ $competition->competition_image }}" name="competition_image">
                    <div class="field">
                        <label for="">Yarışma başlığı</label>
                        <input type="text" placeholder="Yarışma başlığı" name="competition_title" value="{{ $competition->competition_title }}"/>
                    </div>
                    <div class="field">
                        <label for="">Yarışma açıklaması</label>
                        <textarea style="height: 80px" placeholder="Yarışma açıklaması"
                                  name="competition_description">{{ $competition->competition_description }}</textarea>
                    </div>

                    <div class="field">
                        <label for="">Kayıt ücreti</label>
                        <input type="number" placeholder="Kayıt ücreti" name="registration_fee" step="any" value="{{ $competition->registration_fee }}"/>
                    </div>

                    <div class="field">
                        <label for="">Ödül</label>
                        <input type="number" placeholder="Ödül" name="award" step="any" value="{{ $competition->award }}"/>
                    </div>

                    <div class="field">
                        <label for="">Kayıt Durumu</label>
                        <select name="can_register" id="" class="browser-default">
                            <option value="1" @if($competition->can_register === 1) selected @endif>Kayıt Açık</option>
                            <option value="2" @if($competition->can_register === 2) selected @endif>Kayıt Kapalı</option>
                        </select>
                    </div>

                    <div class="field">
                        <label for="">Toplam kazanacak kişi</label>
                        <input type="number" placeholder="Toplam kazanacak kişi" name="total_winner" value="{{ $competition->total_winner }}"/>
                    </div>


                    <div class="field">
                        <label for="">Maximum katılım</label>
                        <input type="number" placeholder="Maximum katılım" name="max_users" value="{{ $competition->max_users }}"/>
                    </div>

                    <div id="start_date" class="field">
                        <label>Başlangıç tarihi/Saati</label>
                        <div class="d-flex d-space-between">
                            <input type="text" name="start_date" class="datepicker mr-1" placeholder="Başlangıç tarihi" value="{{ date_init($competition->start_date) }}"
                                   autocomplete="off">
                            <input type="text" name="start_time" class="timepicker ml-1" placeholder="Başlangıç saati" value="{{ time_init($competition->start_date) }}"
                                   autocomplete="off">
                        </div>
                    </div>
                    ...
                    <div id="start_date" class="field">
                        ...
                        <div class="d-flex d-space-between">
                            <input type="text" name="last_register_date" class="datepicker mr-1"  value="{{ date_init($competition->last_register_date) }}"
                                   placeholder="Son kayıt tarihi" autocomplete="off">
                            <input type="text" name="last_register_time" class="timepicker ml-1"  value="{{ time_init($competition->last_register_date) }}"
                                   placeholder="Son kayıt saati" autocomplete="off">
                        </div>
                    </div>
                    <button type="submit" class="btn btn_custom">Güncelle</button>
                </form>
